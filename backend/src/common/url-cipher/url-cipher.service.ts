...
export class UrlCipherService {
  ...
  generate(payload: UrlCipherPayload) {
    ...
    const hash = crypto
      .createHmac(
        'sha256',
        this.configService.getUrlCipherConfig().urlCipherSecret
      )
      .update(encode)
      .digest('hex');
  ...
  verify(encryptedUrl: EncryptedUrl) {
    const expectedHash = crypto
      ...
      .update(encryptedUrl.encode)
      .digest('hex');
    if (encryptedUrl.hash !== expectedHash) {
      return false;
    }
    const payload: UrlCipherPayload = JSON.parse(
      Buffer.from(encryptedUrl.encode, 'base64').toString('utf8')
    );
