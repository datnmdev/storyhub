  ...
  async getTopViewChartData(atTime: number) {
    ...
    while (atTime >= stopPoint) {
      for (let i = 0; i < stories.length; i++) {
        viewData[i].push(
          await this.getViewCountOfStoryAt(stories[i].id, atTime)
        );
      }
      atTime -= 3600000;
    }
    ...
    let chartData = viewData.map((views, viewsIndex) => {
      return views.map((view, viewIndex) => {
        const total =
          viewData[0][viewIndex] +
          viewData[1][viewIndex] +
          viewData[2][viewIndex];
        if (total === 0) {
          return 0;
        }
    ...
    chartData = chartData.map((views, viewsIndex) => {
      if (viewsIndex === 2) {
        return views.map((view, viewIndex) => {
          const total =
            chartData[0][viewIndex] +
            chartData[1][viewIndex] +
            chartData[2][viewIndex];
          if (total === 0) {
            return 0;
          }
