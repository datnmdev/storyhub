...
import { ForbiddenPageProps } from './ForbiddenPage.type';
import { Link } from 'react-router-dom';
import { getHomePage } from '@components/ErrorBoundary/ErrorBoundary.util';
import { useTranslation } from 'react-i18next';

function ForbiddenPage({ error }: ForbiddenPageProps) {
    ...
    <div className="not-found-container">
      ...
      <div className="center">
        ...
        <div className="circle circle--inner">
          ...
          <div className="four four--2">1</div>
          ...
          <Link className="button" to={getHomePage()}>
            {t('errorPage.button.goHome')}
          </Link>
        </div>
        <div className="sorry">{error.data.message}</div>
      </div>
    </div>
  );
}
