...
import { InputData, InputError, NotesSectionProps } from './NotesSection.type';
...
import { generateValidateSchema } from './NotesSection.schema';
...
function NotesSection({ storyId }: NotesSectionProps) {
      ...
      if (updateStoryResData) {
        ...
        dispatch(
          toastFeature.toastAction.add({
            ...
            title: t('notification.updateNotesSuccess'),
          })
        );
      } else {
        if (updateStoryError) {
          dispatch(
            toastFeature.toastAction.add({
              ...
              title: t('notification.updateNotesFailure'),
            })
          );
        }
      }
  ...
  useEffect(() => {
    if (isEditorReady) {
      notesRef.current.setContent(inputData.notes);
    }
  }, [isEditorReady]);
  ...
  return (
    <div>
      <div className="grow space-y-1">
        <div>
          {t('author.storyManagementPage.uploadStoryPopup.form.notes.title')}
        </div>
          ...
          <div className="grow">
            ...
            {isEditable && (
              <div className="flex justify-end mt-2 space-x-2">
                <Button
                  onClick={() => {
                    notesRef?.current?.setContent(
                      storyData ? storyData[0][0].notes : inputDataInit.notes
                    );
                    setInputData({
                      notes: storyData
                        ? storyData[0][0].notes
                ...
                <Button
                  disabled={errors.notes ? true : false}
                  onClick={async () => {
