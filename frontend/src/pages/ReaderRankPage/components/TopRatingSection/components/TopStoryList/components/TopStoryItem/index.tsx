        ...
        <div className="grow flex flex-col justify-between overflow-hidden">
          ...
          <div className="flex desktop:flex-row tablet:flex-row mobile:flex-col desktop:items-center tablet:items-center mobile:items-start">
            <div className="grow space-x-1">
              <span>
                {t(
                  'reader.rankPage.topRatingSection.topStoryList.item.ratingSummary.ratio.label'
                )}
              </span>
              <span className="font-bold text-red-500">
                {t(
                  'reader.rankPage.topRatingSection.topStoryList.item.ratingSummary.ratio.content',
                  {
                    value: NumberUtils.roundToDecimal(
                      data.ratingSummary.ratingCount > 0
                        ? data.ratingSummary.starsCount /
                            data.ratingSummary.ratingCount
                        : 0,
                      1
                    ),
                  }
            ...
            <div className="grow space-x-1">
              <span>
                {t(
                  'reader.rankPage.topRatingSection.topStoryList.item.ratingSummary.ratingCount.label'
                )}
              </span>
              <span className="font-[450]">
                {t(
                  'reader.rankPage.topRatingSection.topStoryList.item.ratingSummary.ratingCount.content',
                  {
                    value: NumberUtils.formatNumberWithSeparator(
                      String(data.ratingSummary.ratingCount)
                    ),
                  }
                )}
              </span>
